<div class="container mx-auto px-10 py-5">
    <h2 class="text-2xl font-semibold mb-4">Data User</h2>
    <div class="mb-4">
        <a href="?page=user&action=insert">
            <button class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-700">
                Tambah User
            </button>
        </a>
    </div>
    <div class="overflow-x-auto bg-white shadow overflow-y-auto relative">
      <table class="table-auto w-full">
        <thead class="bg-gray-100">
          <tr>
            <th class="px-6 py-2 text-xs text-gray-500 w-5">No.</th>
            <th class="px-6 py-2 text-xs text-gray-500">Username</th>
            <th class="px-6 py-2 text-xs text-gray-500">Role</th>
            <th class="px-6 py-2 text-xs text-gray-500">Actions</th>
          </tr>
        </thead>
        <tbody class="bg-white">
            
            <?php
                $no = 1;
                $sql = "SELECT * FROM users ORDER BY username ASC";
                $result = $conn->query($sql);
                while($row = $result->fetch_assoc()){
            ?>

            <tr class="whitespace-nowrap">
                <td class="px-6 py-4 text-sm text-gray-500"><?= $no++; ?></td>
                <td class="px-6 py-4 text-sm text-gray-500"><?= $row['username']; ?></td>
                <td class="px-6 py-4 text-sm text-gray-500"><?= $row['role']; ?></td>
                
                <td class="px-6 py-4 text-sm text-center">
                    <a href="?page=user&action=update&id=<?= $row['id_user'];?>">
                        <button class="bg-yellow-500 text-white w-16 h-8 rounded hover:bg-yellow-700">
                            Edit
                        </button>
                    </a>
                    <a href="?page=user&action=delete&id=<?= $row['id_user'];?>">
                        <button onclick="return confirm('Yakin ingin menghapus user ini?')" class="bg-red-500 text-white w-16 h-8 rounded hover:bg-red-700">
                            Hapus
                        </button>
                    </a>
                </td>
            </tr>

            <?php 
                }
                $conn->close();
            ?>
        <!-- More rows as needed -->
        </tbody>
      </table>
    </div>
  </div>